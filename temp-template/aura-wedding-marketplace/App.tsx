...
export default function App() {
  ...
  const scrollToSection = (id: TabType) => {
    ...
    if (element) {
      // Offset for the sticky header + tabs
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <nav className="h-16 bg-white/90 backdrop-blur-md border-b border-stone-100 sticky top-0 z-50 flex items-center justify-between px-4 md:px-8">
        <div className="text-xl font-serif font-extrabold tracking-tight text-stone-900 flex items-center">
          Svatbot<span className="text-primary-600">.</span>
        </div>
        <div className="flex gap-6">
           <button className="text-sm font-bold text-stone-600 hover:text-primary-600 hidden md:block transition-colors">Jak to funguje</button>
           <button className="text-sm font-bold text-stone-600 hover:text-primary-600 hidden md:block transition-colors">Hledám dodavatele</button>
