...
interface SimpleRoomImageUploadProps {
  images: string[]
...
export default function SimpleRoomImageUpload({
  images,
  onImagesChange,
  maxImages = 10,
  disabled = false
}: SimpleRoomImageUploadProps) {
  const { user } = useAuth()
  ...
  const handleFiles = (files: FileList | null) => {
    ...
    // Auto-upload images
    newPendingImages.forEach((pendingImage, index) => {
      uploadImage(pendingImages.length + index, pendingImage)
    })
  }
  ...
  const removeImage = async (imageUrl: string, index: number) => {
    try {
      ...
      console.log('✅ Obrázek pokoje smazán ze Storage')
    } catch (error) {
    ...
    <div className="space-y-4">
      ...
      {images.length === 0 && pendingImages.length === 0 && (
        <div className="text-center py-8 text-gray-500">
          ...
          <p className="text-sm">Zatím nemáte žádné fotky pokoje</p>
        </div>
      )}
    </div>
  )
