...
import { Clock, Calendar, Settings, Download, Sparkles, Plus, Edit3, Trash2, RotateCcw, ArrowLeft, Loader2, AlertCircle, List, BarChart3, Save, X } from 'lucide-react'
...
export default function AITimelinePage() {
  const { generateTimeline, loading: aiLoading, error: aiError } = useAI()
  const {
    timeline,
    settings,
    loading,
  ...
  const handleGenerateTimeline = async () => {
    try {
      ...
      if (result?.timeline) {
        await generateFromAI(result.timeline)
      }
    } catch (err) {
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-3">
              ...
              >
                ...
                <span>{(aiLoading || loading) ? 'Generuji...' : 'AI Timeline'}</span>
              </button>
            ...
            {(error || aiError) && (
              <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <p className="text-sm text-red-600">{error || aiError}</p>
                    <button
                      onClick={clearError}
            ...
            {timeline.length === 0 && !loading && (
              <div className="wedding-card text-center py-12">
                ...
                <p className="text-gray-600 mb-6">
                  Začněte vytvořením AI timeline pro váš svatební den
                </p>
                <button
                  onClick={handleGenerateTimeline}
                  disabled={aiLoading}
                ...
                >
                  ...
                  <span>{aiLoading ? 'Generuji...' : 'Vygenerovat Timeline'}</span>
                </button>
              </div>
            )}
